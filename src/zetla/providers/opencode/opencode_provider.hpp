...
namespace zetla::providers {
    ...
    class OpenCodeProvider : public IProviderFacade, public RouteProviderMixin<OpenCodeProvider> {
    public:
        static OpenCodeProvider configure(const std::string& api_key, const std::string& base_url = "") {
            OpenCodeProvider p;
            p.api_key_ = api_key;
            std::string url = base_url.empty() ? "https://opencode.ai/zen/v1" : base_url;

            route::RouteDefaults d;
            d.model_prefix_to_strip = "opencode-zen/";
        ...
        std::string provider_id() const override { return "opencode_zen"; }
        std::string provider_name() const override { return "OpenCode Zen"; }
